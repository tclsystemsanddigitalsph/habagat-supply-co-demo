...
(function (global) {
  ...
  function header() {
    ...
    var mLinks = NAV.map(function (n) { return '<li><a href="' + n.href + '">' + esc(n.label) + '</a></li>'; }).join("") +
      '<li><a href="cart.html">Cart (' + count + ')</a></li>' +
      '<li><a class="mnav-admin" href="admin.html">Demo Admin</a></li>';

    var initials = esc((s.storeName || "H").replace(/[^A-Za-z]/g, "").slice(0, 2).toUpperCase() || "HB");
      ...
      '<header class="site-header">' +
        '<div class="wrap header-inner">' +
          '<a class="brand" href="index.html">' +
            ...
            '<span><span class="brand-name">' + esc(s.storeName) + '</span>' +
            '<span class="brand-sub">' + esc(s.tagline) + '</span></span>' +
          '</a>' +
        ...
        '<div class="wrap footer-grid">' +
          ...
          '<div>' +
            ...
            '<ul>' +
              ...
              '<li><a href="tel:' + esc(String(s.phone).replace(/\s/g, "")) + '">' + esc(s.phone) + '</a></li>' +
              '<li>' + esc(s.address) + '</li>' +
            '</ul>' +
          '</div>' +
        '</div>' +
        '<div class="wrap footer-bottom">' +
          '<span>&copy; ' + year + ' ' + esc(s.storeName) + '. Demo site &mdash; no real orders are processed.</span>' +
  ...
  function productCard(p) {
    var options = p.options && p.options.values && p.options.values.length
      ? '<span class="opt">' + p.options.values.length + ' ' + esc(p.options.label.toLowerCase()) + ' options</span>'
      : '';
    var badge = !p.active
      ? '<span class="badge-off badge-out">Out of stock</span>'
      : (p.featured ? '<span class="badge-off">Featured</span>' : '');

    return '' +
      '<article class="card">' +
        '<a class="card-thumb" href="product.html?id=' + encodeURIComponent(p.id) + '" aria-label="' + esc(p.name) + '">' +
          '<img src="' + p.art + '" alt="' + esc(p.name) + '" loading="lazy" width="240" height="240">' +
        '</a>' +
        '<div class="card-body">' +
          ...
          '<div class="card-price">' + badge + S.pesoShort(p.price) + options + '</div>' +
        '</div>' +
        '<div class="card-foot">' +
          (p.active
            ? '<a class="btn btn-outline btn-sm btn-block" href="product.html?id=' + encodeURIComponent(p.id) + '">View details</a>'
  ...
  function syncCartCount() {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (count) nodes[i].removeAttribute("hidden"); else nodes[i].setAttribute("hidden", "");
    }
    var mobileCart = global.document.querySelector(".mobile-nav a[href='cart.html']");
    if (mobileCart) mobileCart.textContent = "Cart (" + count + ")";
  }
  ...
  function toast(msg) {
    ...
    toastTimer = global.setTimeout(function () { toastEl.classList.remove("show"); }, 2600);
  }
  ...
  function mount() {
    ...
    syncCartCount();
    global.addEventListener("habagat:change", syncCartCount);
  }

  global.HabagatUI = {
    esc: esc, mount: mount, productCard: productCard,
    header: header, footer: footer, toast: toast, syncCartCount: syncCartCount
  };
})(window);
